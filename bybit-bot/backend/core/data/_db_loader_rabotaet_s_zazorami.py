...
def fill_missing_candles(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    if df.empty or "timestamp" not in df.columns:
        print("[DEBUG] fill_missing_candles: пустой df или нет timestamp")
        return df

    interval_sec = TIMEFRAMES_CONFIG[timeframe]["interval_sec"]

    # Оставляем timestamp в datetime64
    df["timestamp"] = pd.to_datetime(df["timestamp"], unit="s", utc=True)

    # Строим временной диапазон
    ...
    # Только теперь переводим обратно в UNIX
    df_full["timestamp"] = df_full["timestamp"].apply(lambda dt: int(dt.timestamp()))

    print("[DEBUG] fill_missing_candles: после вставки пустых, shape:", df_full.shape)
    print("[DEBUG] fill_missing_candles: head:\n", df_full.head(3))
